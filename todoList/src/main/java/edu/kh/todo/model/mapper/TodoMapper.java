 ...
 * - MyBatics 제공 어노테이션
 * 
 * - MyBatics 해당 인터페이스를 DAO로 인식하여 SQL 매핑을 처리
 * 
 * - 해당 어노테이션이 작성된 인터페이스는
 * 	 namespace에 해당 인터페이스가 작성쇰
 * 	 mapper.xml 파일과 연결되어 SQL 호출/수행/결과 반환 가능
 * 
 * - Mybatics에서 제공하는 Mapper 상속 객체가 Bean으로 등록됨
 * 
 * - @Mapeer를 사용할 때 @Repository는 필요하지 않음
 * 	 -> Mybatics가 DAO 객체로 Mapper 인터페이스를 관리하기 때문
 */
